...
local exile_basket = isExile and minetest.registered_nodes['tech:woven_storage_basket']
local exile_on_construct = function (pos, w, h) return exile_basket and exile_basket.on_construct(pos,w,h) end
...
local get_bench_def = function ()
  ...
  if isExile then def.stack_max = minimal.stack_max_bulky * 2 end
  return def
end

-- furniture; possible replacement: 3dforniture:chair
minetest.register_node("cottages:bench", get_bench_def())
--[[ {
  drawtype = "nodebox",
  description = S("simple wooden bench"),
  tiles = {"cottages_minimal_wood.png", "cottages_minimal_wood.png",  "cottages_minimal_wood.png",  "cottages_minimal_wood.png",  "cottages_minimal_wood.png",  "cottages_minimal_wood.png"},
...
minetest.register_node("cottages:shelf", {
  ...
  on_construct = (isExile and exile_on_construct) or function(pos)
  ...
  after_place_node = function(pos, placer, itemstack, pointed_thing)
    if isExile then exile_on_construct(pos, 8, 4) end
  end,

  on_receive_fields = function (pos, formname, fields, sender, width, height)
    if not (isExile) then return end -- no fields to receive if not exile
    local label = fields.label
    if label then
      local meta = minetest.get_meta(pos)
      local cleanlabel = minimal.sanitize_string(label)
      meta:set_string('label', cleanlabel)
      exile_on_construct(pos, width, height)
    end
  end,
  can_dig = function( pos,player )
    ...
    if name and not is_owner(pos, name) then return false end
  ...
  on_metadata_inventory_take = function(pos, listname, index, stack, player)
    local  meta = minetest.get_meta( pos );
    local  inv = meta:get_inventory();
    if( not(isExile) and inv:is_empty("main")) then
